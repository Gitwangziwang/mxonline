...
class MyfavCourserView(LoginRequireMixin,View):
    def get(self,request):
        course_list = []
...
class MyMessageView(LoginRequireMixin,View):
    def get(self,request):
        ...
        try:
            page = request.GET.get('page', 1)           # 对我的消息进行分页
        except PageNotAnInteger:
            page = 1

        p = Paginator(all_message, 5, request=request)     #每页５个
...
class IndexView(View):
    def get(self,request):
        ...
        courses = Course.objects.filter(is_banner=False)[:5]  #非轮播的课程
